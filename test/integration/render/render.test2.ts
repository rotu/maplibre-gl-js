 ...
 */
async function getImageFromStyle(style: StyleWithTestData) {
    ...
    const tog = await page.addScriptTag({
        type: 'module',
    ...
    `, style);
    assert(map);
    const image = await page.locator('body').screenshot();
    return image;
...
for (const [index, style] of testStyles.entries()) {
    test(`test${index}`, async () => {
        const data =     getImageFromStyle(style);
    });
